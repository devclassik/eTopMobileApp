...
public class BankInfo extends AppCompatActivity {
    ...
    public static int bankControl = 0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toolbar.setNavigationOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                intent.setClass(BankInfo.this, PersonalInformation.class);
        ...
        if(BankInfo.bankControl == 1)
        {
            ...
            BankInfo.bankControl = 0;
        }

    }

    public void Proceed(String[] option, String[] code) {
        SignupVr.bvn = bvn.getEditText().getText().toString().trim();
        SignupVr.accountholder = accountholder.getEditText().getText().toString().trim();
        SignupVr.accountnumber = accountnumber.getEditText().getText().toString().trim();
        Log.i(TAG, "ACCT NUMBER: " + SignupVr.accountholder + ".");
        String action = bankname.getText().toString();
        if (!action.isEmpty()) {
            SignupVr.bankname = action;
            int i = Arrays.asList(option).indexOf(action);
            SignupVr.bankcode = code[i];
        }

        if (SignupVr.bvn.isEmpty()) {
            bvn.setFocusable(true);
            Toast.makeText(BankInfo.this, "Please Provide Bvn", Toast.LENGTH_LONG).show();
        } /*else if (SignupVr.accountholder.isEmpty()) {
            accountholder.setFocusable(true);
            Toast.makeText(BankInfo.this, "Please Account Name", Toast.LENGTH_LONG).show();
        } */else if (SignupVr.accountnumber.isEmpty()) {
            accountnumber.setFocusable(true);
            Toast.makeText(BankInfo.this, "Please Provide Account Number", Toast.LENGTH_LONG).show();
        } else if (SignupVr.bankname.isEmpty()) {
            bankname.setFocusable(true);
            Toast.makeText(BankInfo.this, "Select Bank Name", Toast.LENGTH_LONG).show();
        } else {
            ...
            }else
            {
                ...
                SignupVr.longitude = String.valueOf(longitude);
                SignupVr.latitude = String.valueOf(latitude);
            }
            ...
            if(control == 0)
            {
                try {
                    ...
                    obj.put("bankcode", SignupVr.bankcode);
                    obj.put("destination", SignupVr.accountnumber);
                    Log.i(TAG, "VALIDATION REQUEST: " + obj.toString());
                    new validateAccount().execute(BASEURL + "/apis/etop/kuda/bankvalidation/", obj.toString());
                } catch (JSONException e) {
                    e.printStackTrace();
                }
            }else
            {
                ...
                inte.setClass(BankInfo.this, Setpassword.class);
                startActivity(inte);
                finish();
            }
        }
    ...
    public void onBackPressed() {
        Intent intent = new Intent();
        PersonalInformation.personalControl = 1;
        intent.setClass(BankInfo.this, Login.class);
        startActivity(intent);
        finish();
    }
}